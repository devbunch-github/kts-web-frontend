...
import { fetchAccountantExpenseById, updateAccountantExpense } from "../../api/accountantdashboard";

export default function AccountantExpenseEdit() {
  const { id } = useParams();
  ...
  const loadExpense = async () => {
    try {
      const res = await fetchAccountantExpenseById(id);
      setForm({
        Supplier: res.Supplier || "",
        Amount: res.Amount || "",
        PaidDateTime: res.PaidDateTime
          ? res.PaidDateTime.split("T")[0]
          : "",
        Notes: res.Notes || "",
        reason: "",
      });
    } catch {
      showAlert("error", "Failed to load expense record.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await updateAccountantExpense(id, form);
      showAlert("success", res.message);
      setTimeout(() => navigate("/accountant/expense"), 1500);
    } catch {
      showAlert("error", "Failed to update expense record.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#fffaf6] p-6 font-[Poppins,sans-serif]">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-8">
        <h2 className="text-2xl font-semibold text-gray-800 flex items-center gap-2 mb-6">
          <Pencil className="w-6 h-6 text-[#f28c38]" /> Edit Expense
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate("/accountant/expense")}
