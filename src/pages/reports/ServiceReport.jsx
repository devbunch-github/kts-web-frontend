...
export default function ServiceReport() {
  ...
  async function fetchReport() {
    try {
      setLoading(true);
      const res = await http.get("/api/business/reports/service", {
          ...
          <table className="w-full text-sm text-gray-700">
            <thead>
              <tr className="text-left border-b border-gray-100 text-gray-500">
                ...
                <th className="pb-3">Booked Services</th>
                <th className="pb-3">Total Income</th>
                <th className="pb-3">Profit/min</th>
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, limit).map((r, idx) => (
                ...
                >
                  ...
                  <td>{r.duration}</td>
                  <td>{r.booked_services}</td>
                  <td>£ {Number(r.total_income || 0).toFixed(2)}</td>
                  <td>£ {Number(r.profit_per_min || 0).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
