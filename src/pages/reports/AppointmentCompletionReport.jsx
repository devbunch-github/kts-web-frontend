...
export default function AppointmentCompletionReport() {
  ...
  async function fetchReport() {
    try {
      setLoading(true);
      const res = await http.get("/api/business/reports/appointment-completion", {
  ...
  const filtered = reports.filter((r) =>
    r.date.toLowerCase().includes(filters.search.toLowerCase())
  );
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-[70vh] text-gray-500 text-sm">
        ...
        Loading appointment completion report...
      </div>
    );
    ...
    <div className="bg-[#faf7f7] min-h-screen p-6 md:p-10 space-y-8">
      ...
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-semibold text-gray-800">
          Reports / Appointment Completion Report
        </h1>
      </div>
          ...
          <table className="w-full text-sm text-gray-700">
            <thead>
              <tr className="text-left border-b border-gray-100 text-gray-500">
                ...
                <th className="pb-3">Booked</th>
                <th className="pb-3">Completed</th>
                <th className="pb-3">Canceled</th>
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, limit).map((r, idx) => (
                ...
                >
                  ...
                  <td>{r.booked.toString().padStart(2, "0")}</td>
                  <td>{r.completed.toString().padStart(2, "0")}</td>
                  <td>{r.canceled.toString().padStart(2, "0")}</td>
                </tr>
              ))}
            </tbody>
          </table>
