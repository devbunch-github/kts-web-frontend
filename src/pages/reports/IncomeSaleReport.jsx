...
export default function IncomeSaleReport() {
  ...
  async function fetchData() {
    try {
      setLoading(true);
      const res = await http.get("/api/business/reports/income-sale", {
          ...
          <table className="w-full text-sm text-gray-700">
            ...
            <tbody>
              {filtered.slice(0, limit).map((r, i) => (
                <tr
                  key={i}
                  className="border-b border-gray-50 hover:bg-rose-50/40 transition"
                >
                  ...
                  <td className="text-right font-medium">
                    ...
                    {r.is_refund && (
                      <span className="ml-2 text-[10px] px-2 py-[1px] rounded-full bg-rose-100 text-rose-700 font-medium">
                        Refund
                      </span>
                    )}
                  </td>
